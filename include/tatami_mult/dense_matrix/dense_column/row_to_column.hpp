...
template<
    bool is_pointer_,
    typename RightValue_, // not used if `is_pointer_ = true`.
    typename LeftValue_,
    typename LeftIndex_,
    typename RightColumns_,
    typename RightMatrix_, // not used if `is_pointer_ = true`.
    typename GetRightRow_, // not used if `is_pointer_ = false`.
    typename Output_
>
