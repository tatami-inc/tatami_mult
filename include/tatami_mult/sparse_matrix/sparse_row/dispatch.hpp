#ifndef TATAMI_MULT_SPARSE_MATRIX_SPARSE_ROW_DISPATCH_HPP
#define TATAMI_MULT_SPARSE_MATRIX_SPARSE_ROW_DISPATCH_HPP
...
/**
 * @brief Options for `multiply_sparse_row_with_sparse_matrix()`.
 */
struct MultiplySparseRowWithSparseMatrixOptions {
     ...
     */
    MultiplySparseRowWithSparseColumnMatrixToColumnOutputOptions column_to_column;
     ...
     */
    MultiplySparseRowWithSparseColumnMatrixToRowOutputOptions column_to_row;

    /**
     * Options to pass to `multiply_sparse_row_with_sparse_row_matrix_to_column_output()`,
     * if `right` is a row-major matrix and `output_row_major == false`.
     */
    MultiplySparseRowWithSparseRowMatrixToColumnOutputOptions row_to_column;

    /**
     * Options to pass to `multiply_sparse_row_with_sparse_row_matrix_to_row_output()`,
     * if `right` is a row-major matrix and `output_row_major == true`.
     */
    MultiplySparseRowWithSparseRowMatrixToRowOutputOptions row_to_row;
};
 ...
 */
inline void set_num_threads(MultiplySparseRowWithSparseMatrixOptions& options, int num_threads) {
 ...
 */
inline void set_sparse_block_size(MultiplySparseRowWithSparseMatrixOptions& options, int block_size) {
    options.column_to_column.block_size = block_size;
    options.column_to_row.block_size = block_size;
}

/**
 * This function delegates to `multiply_sparse_row_with_sparse_row_matrix_to_row_output()`,
 * `multiply_sparse_row_with_sparse_row_matrix_to_column_output()`,
 * `multiply_sparse_row_with_sparse_column_matrix_to_row_output()`, or
...
void multiply_sparse_row_with_sparse_matrix(
    ...
    if (right.prefer_rows()) {
        if (output_row_major) {
            multiply_sparse_row_with_sparse_row_matrix_to_row_output(left, right, output, options.row_to_row);
        } else {
            multiply_sparse_row_with_sparse_row_matrix_to_column_output(left, right, output, options.row_to_column);
        }

    } else {
        if (output_row_major) {
            multiply_sparse_row_with_sparse_column_matrix_to_row_output<accumulators_>(left, right, output, options.column_to_row);
        } else {
            multiply_sparse_row_with_sparse_column_matrix_to_column_output<accumulators_>(left, right, output, options.column_to_column);
        }
    }
}
