...
To avoid this, we process a block of $B$ sparse vectors for each dense vector.
...
Larger values of $B$ improve speed at the cost of increased main memory usage as **tatami** needs to realize all sparse vectors into memory.

Alternatively, if the dense vectors are small, we might consider loading a block of $D$ dense vectors into cache at once.
