...
If the dense vectors are small, we might consider loading a block of \f$C\f$ dense vectors into cache at once.
...
By caching both the dense and sparse vectors, this strategy is more effective than just caching one dense vector. 
However, this is only applicable when the extent of the relevant dimension is small.
