...
TEST_P(MultipleVectorsSparseRowTest, Basic) {
    ...
    auto dump = tatami_test::simulate_vector<double>(NR * NC, [&]{
        ...
        opt.lower = -10;
        opt.upper = 10;
        opt.seed = 71 + NR + NC + NRHS + block_size + nthreads;
        return opt;
    }());
...
class MultipleVectorsSparseRowEmptyTest : public ::testing::TestWithParam<std::tuple<int, int, int, int, int, int> > {};

TEST_P(MultipleVectorsSparseRowEmptyTest, Basic) {
    const auto params = GetParam();
    const int NR = std::get<0>(params);
    const int NC = std::get<1>(params);
    const int NRHS = std::get<2>(params);
...
INSTANTIATE_TEST_SUITE_P(
    MultipleVectors,
    MultipleVectorsSparseRowEmptyTest,
    ::testing::Combine(
        ::testing::Values(100, 33), // number of rows.
        ::testing::Values(59, 148), // number of columns.
        ::testing::Values(20, 74),  // number of RHS vectors.
