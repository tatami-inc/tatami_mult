...
TEST_P(DenseMatrixDenseRowTest, Basic) {
    ...
    auto dump = tatami_test::simulate_vector<double>(NR * NC, [&]{
        ...
        opt.lower = -10;
        opt.upper = 10;
        opt.seed = 690 + NR + NC + NRHS + blocks.first + blocks.second + nthreads;
        return opt;
    }());
    ...
    auto rhs = tatami_test::simulate_vector<double>(NC * NRHS, [&]{
        ...
        opt.lower = -10;
        opt.upper = 10;
        opt.seed = 142 + NR + NC + NRHS + blocks.first + blocks.second + nthreads;
        return opt;
    }());
...
TEST(DenseMatrixDenseRow, Options) {
    tatami_mult::MultiplyDenseRowWithDenseMatrixOptions opt;
