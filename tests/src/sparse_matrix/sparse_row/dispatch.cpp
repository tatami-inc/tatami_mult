...
#include "tatami_mult/sparse_matrix/sparse_row/dispatch.hpp"

#include "../../utils.h"

class SparseMatrixSparseRowTest : public ::testing::TestWithParam<std::tuple<int, int, int, int, int> > {};

TEST_P(SparseMatrixSparseRowTest, Basic) {
    ...
    auto dump = tatami_test::simulate_vector<double>(NR * NC, [&]{
        ...
        opt.lower = -10;
        opt.upper = 10;
        opt.seed = 693 + NR + NC + NRHS + block_size + nthreads;
        return opt;
    }());
    ...
    auto rhs = tatami_test::simulate_vector<double>(NC * NRHS, [&]{
        ...
        opt.lower = -10;
        opt.upper = 10;
        opt.seed = 142 + NR + NC + NRHS + block_size + nthreads;
        return opt;
    }());
    ...
    tatami_mult::MultiplySparseRowWithSparseMatrixOptions opt;
    ...
    tatami_mult::multiply_sparse_row_with_sparse_matrix<1>(*sparse_row, *right_col, sr_rc_ro1.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<4>(*sparse_row, *right_col, sr_rc_ro4.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<1>(*sparse_row, *right_col, sr_rc_co1.data(), false, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<4>(*sparse_row, *right_col, sr_rc_co4.data(), false, opt);

    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_row, *right_row, sr_rr_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_row, *right_row, sr_rr_co.data(), false, opt);

    // Checking that it still works for column-major LHS.
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_row, sc_rr_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_col, sc_rc_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_row, sc_rr_co.data(), false, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_col, sc_rc_co.data(), false, opt);
...
class SparseMatrixSparseRowEmptyTest : public ::testing::TestWithParam<std::tuple<int, int, int, int, int, int> > {};

TEST_P(SparseMatrixSparseRowEmptyTest, Empty) {
    const auto params = GetParam();
    const int NR = std::get<0>(params);
    const int NC = std::get<1>(params);
    const int NRHS = std::get<2>(params);
    ...
    auto dump = simulate_strided_sparse_matrix(NR, NC, stride, /* seed = */ 453 + NR + NC + NRHS + block_size + stride + nthreads);
    auto sparse_row = tatami::convert_to_compressed_sparse<double, int>(tatami::DenseRowMatrix<double, int>(NR, NC, dump), true, {});
    auto sparse_col = tatami::convert_to_compressed_sparse<double, int>(*sparse_row, false, {});
    ...
    // Checking different choices of accumulators.
    tatami_mult::multiply_sparse_row_with_sparse_matrix<1>(*sparse_row, *right_col, dr_rc_ro1.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<4>(*sparse_row, *right_col, dr_rc_ro4.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<1>(*sparse_row, *right_col, dr_rc_co1.data(), false, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix<4>(*sparse_row, *right_col, dr_rc_co4.data(), false, opt);

    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_row, *right_col, dr_rr_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_row, *right_col, dr_rr_co.data(), false, opt);

    // Checking it works with a column-major RHS.
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_row, dc_rr_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_row, dc_rr_co.data(), false, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_col, dc_rc_ro.data(), true, opt);
    tatami_mult::multiply_sparse_row_with_sparse_matrix(*sparse_col, *right_col, dc_rc_co.data(), false, opt);
...
INSTANTIATE_TEST_SUITE_P(
    SparseMatrix,
    SparseMatrixSparseRowEmptyTest,
    ::testing::Combine(
        ::testing::Values(98, 36), // number of rows.
        ::testing::Values(35, 104), // number of columns.
        ::testing::Values(11, 46),  // number of RHS vectors.
...
TEST(SparseMatrixSparseRow, Options) {
    tatami_mult::MultiplySparseRowWithSparseMatrixOptions opt;
